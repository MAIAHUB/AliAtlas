...
// Adapter for the external vision model ("b.ai"). Configured only on the server:
...
export function visionStatus() {
  ...
  return {
    ...
    provider: 'b.ai',
    message: configured
      ? 'b.ai is connected. Suggestions require review.'
      : 'Add BAI_API_KEY, BAI_BASE_URL and BAI_MODEL to the server environment to enable abnormality detection.',
  };
}
...
export function parseModelJson(content) {
  ...
  try {
    ...
    throw new AppError('b.ai returned an unreadable answer. Try again.', 502, 'AI_BAD_RESPONSE');
  }
}
...
async function callOpenAiStyle(c, content, system = PROMPT) {
  ...
  try {
    ...
    throw new AppError('b.ai is unreachable or took too long.', 502, 'AI_UNAVAILABLE');
  }
  if (response.status === 401 || response.status === 403)
    throw new AppError('b.ai rejected the API key.', 502, 'AI_AUTH');
  if (response.status === 429)
    throw new AppError('b.ai rate limit reached. Wait a moment and retry.', 429, 'AI_RATE_LIMIT');
  if (!response.ok) throw new AppError(`b.ai returned an error (${response.status}).`, 502);
...
export async function draftReport({ reportType, technique, findingsText }) {
  ...
  assert(draft.impression, 'b.ai returned an empty report. Try again.', 502, 'AI_BAD_RESPONSE');
