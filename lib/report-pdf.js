...
export function buildReportPdf({ study, report, findings, images, generatedAt = new Date() }) {
  ...
  if (!confirmed.length) {
    ...
    confirmed.forEach(({ f, series, index, total, approx }, i) => {
      ...
      const source = f.source === 'ai' ? ' (auto-detected, confirmed)' : '';
      row([
        String(i + 1),
  ...
  const disclaimer = diagnostic
    ? 'Generated with AliAtlas, an educational workspace. Automated suggestions and drafts are not a diagnosis and were reviewed by the reader named in this report.'
