...
import { parseDicom, seriesKey, segmentationEligibility } from './dicom.js';
...
export async function importStudy(request, owner) {
  assert(
  ...
  try {
    const incoming = await receiveFiles(request, temp),
      files = [];
    ...
    return { studies: result, importedFrames: frameCount, skipped, warnings };
  } finally {
