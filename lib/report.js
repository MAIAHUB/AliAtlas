...
// Impression from the measurements alone: one numbered line per confirmed finding, in slice order.
...
// Every size number of every confirmed finding must appear unchanged in drafted text.
...
// Fills every section. Technique comes from the DICOM (exact); the drafting service writes findings prose,
// impression, limitations and recommendation from the measured findings (text only).
// Drafted findings text that alters a measurement is replaced by the measured text. Clinical
...
export async function generateReportDraft(study, findings, current, draftReport) {
  ...
  try {
    ...
    if (ai.findingsText && keepsMeasurements(ai.findingsText, findings))
      ...
      draft.note =
        'The drafted findings text did not keep every measurement exactly, so the measured findings were used.';
    // Plain CT must always state its limitation, whatever the draft says.
    ...
    draft.note = `Automatic drafting was unavailable (${e.message}); templates were used.`;
  }
  return draft;
}
...
export async function updateReport(owner, id, input, user) {
  return withLock(studyDir(owner, id), async () => {
    ...
    if (input.status === 'final') {
      ...
      assert(
        !pending,
        `Review ${pending} automated suggestion${pending === 1 ? '' : 's'} before finalizing.`,
        409,
        'UNREVIEWED_FINDINGS',
      );
